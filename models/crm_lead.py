...
class CRMLead(models.Model):
        ...
        try:
            ...
            if whatsapp_instance_id.provider == "whatsapp_chat_api":
                if whatsapp_instance_id.send_whatsapp_through_template:
                    ...
                    res.send_message_on_whatsapp_chat_api(whatsapp_instance_id)
    ...
    def send_message_on_whatsapp_chat_api(self, whatsapp_instance_id):
            ...
            if res_partner_id.country_id.phone_code and res_partner_id.mobile:
                ...
                if self.description:
                    ...
                    if santized_description:
                        msg += '\n*' + _("Description") + ':* ' + str(santized_description)
                msg = _('Hello') + ' ' + res_partner_id.name + ',' + '\n' + _("New lead assigned to you") + '\n*' + _("Lead name") + ':* ' + self.name + "" + msg
                if whatsapp_instance_id.crm_lead_add_signature:
                ...
                if response.status_code == 201 or response.status_code == 200:
                    ...
                    if not json_send_message_response.get('sent') and json_send_message_response.get('error') and json_send_message_response.get(
                        ...
                        self.env['whatsapp.msg'].create_whatsapp_message(mobile_with_country, msg, json_send_message_response.get('id'),
                                                                         ...
                                                                         "text", whatsapp_instance_id, 'crm.lead', self)
                        mail_message_obj = self.env['mail.message']
                        if whatsapp_instance_id.crm_lead_add_message_in_chatter:
    ...
    def send_message_on_whatsapp_through_template(self, whatsapp_instance_id):
        ...
        headers = {"Content-Type": "application/json"}
        res_partner_id = self.user_id.partner_id
        whatsapp_msg_number = res_partner_id.mobile
        if whatsapp_msg_number:
            whatsapp_msg_number_without_space = whatsapp_msg_number.replace(" ", "")
            message = ''
            ...
            if not whatsapp_instance_id.crm_lead_add_signature:
                response = self.chat_api_template_crm_lead_without_signature(url, headers, res_partner_id, number_without_code, whatsapp_instance_id)
                message = self.create_crm_lead_message(res_partner_id, whatsapp_instance_id, signature=False)
            else:
                response = self.chat_api_template_crm_lead_with_signature(url, headers, res_partner_id, number_without_code, whatsapp_instance_id)
                message = self.create_crm_lead_message(res_partner_id, whatsapp_instance_id, signature=True)
            if response.status_code == 201 or response.status_code == 200:
    ...
    def chat_api_template_crm_lead_without_signature(self, url, headers, res_partner_id, number_without_code, whatsapp_instance_id):
        whatsapp_template_id = self.env['whatsapp.templates'].sudo().search(
            [('name', '=', 'crm_lead_without_signature_' + whatsapp_instance_id.sequence), ('whatsapp_instance_id', '=', whatsapp_instance_id.id), ('default_template', '=', True)])
        customer_name = ''
        if self.partner_id:
        ...
        if self.date_deadline:
            expected_closing_date = str(self.date_deadline)
        else:
            expected_closing_date = ' '
        description = ''
        if self.description:
            santized_description = self.env['project.task'].cleanhtml(self.description)
            if santized_description:
                ...
                "params": [
                    {
                        "type": "body",
                        "parameters": [
                            {"type": "text", "text": res_partner_id.name},
                            {"type": "text", "text": self.name},
                            {"type": "text", "text": customer_name},
                            {"type": "text", "text": email},
                            {"type": "text", "text": phone},
                            {"type": "text", "text": expected_closing_date},
                            {"type": "text", "text": description},
                        ]
                    }
                ],
                "phone": str(res_partner_id.country_id.phone_code) + "" + number_without_code
            }
    ...
    def chat_api_template_crm_lead_with_signature(self, url, headers, res_partner_id, number_without_code, whatsapp_instance_id):
        whatsapp_template_id = self.env['whatsapp.templates'].sudo().search(
            [('name', '=', 'crm_lead_with_signature_' + whatsapp_instance_id.sequence), ('whatsapp_instance_id', '=', whatsapp_instance_id.id), ('default_template', '=', True)], limit=1)
        customer_name = ''
        if self.partner_id:
            customer_name = self.partner_id.name
        else:
    ...
    def send_message_from_gupshup_through_template(self, whatsapp_instance_id):
        ...
        if whatsapp_msg_number:
            ...
            self.env['whatsapp.msg'].get_add_in_opt_in_user(whatsapp_instance_id, headers, number_without_code, self)

            if not whatsapp_instance_id.crm_lead_add_signature:
                response = self.gupshup_template_crm_lead_without_signature(url, headers, res_partner_id, number_without_code, whatsapp_instance_id)
                message = self.create_crm_lead_message(res_partner_id, whatsapp_instance_id, signature=False)
            else:
                response = self.gupshup_template_crm_lead_with_signature(url, headers, res_partner_id, number_without_code, whatsapp_instance_id)
                message = self.create_crm_lead_message(res_partner_id, whatsapp_instance_id, signature=True)
            if response.status_code == 201 or response.status_code == 200 or response.status_code == 202:
                ...
                self.env['whatsapp.msg.res.partner'].with_context({'partner_id': res_partner_id.id}).gupshup_create_whatsapp_message(
                    str(res_partner_id.country_id.phone_code) + number_without_code, message, json_response.get('messageId'), whatsapp_instance_id, 'crm.lead',
                    self)
                if whatsapp_instance_id.crm_lead_add_message_in_chatter:
                    mail_message_obj = self.env['mail.message']
                    mail_message_body = """<p style='margin:0px; font-size:13px; font-family:"Lucida Grande", Helvetica, Verdana, Arial, sans-serif'><img src="/web_editor/font_to_img/62002/rgb(73,80,87)/13" data-class="fa fa-whatsapp" style="border-style:none; vertical-align:middle; height:auto; width:auto" width="0" height="0"></p>"""
    ...
    def gupshup_template_crm_lead_without_signature(self, url, headers, res_partner_id, number_without_code, whatsapp_instance_id):
        whatsapp_template_id = self.env['whatsapp.templates'].sudo().search(
            [('name', '=', 'crm_lead_without_signature_' + whatsapp_instance_id.sequence), ('whatsapp_instance_id', '=', whatsapp_instance_id.id),
        ...
        if whatsapp_template_id.approval_state == 'APPROVED':
            ...
            payload = {
                ...
                "destination": str(res_partner_id.country_id.phone_code) + "" + number_without_code,
                "template": json.dumps(
                    {
    ...
    def gupshup_template_crm_lead_with_signature(self, url, headers, res_partner_id, number_without_code, whatsapp_instance_id):
        whatsapp_template_id = self.env['whatsapp.templates'].sudo().search(
            [('name', '=', 'crm_lead_with_signature_' + whatsapp_instance_id.sequence), ('whatsapp_instance_id', '=', whatsapp_instance_id.id),
             ('default_template', '=', True)], limit=1)
        if whatsapp_template_id.approval_state == 'APPROVED':
            customer_name = ''
            if self.partner_id:
    ...
    def create_crm_lead_message(self, res_partner_id, whatsapp_instance_id, signature):
        message = ''
        if self.partner_id:
            message += "\n*" + _("Customer") + ':* ' + self.partner_id.name
        if self.email_from:
        ...
        if self.description:
            santized_description = self.env['project.task'].cleanhtml(self.description)
            if santized_description:
                message += '\n*' + _("Description") + ':* ' + santized_description
        message = _('Hello') + ' ' + res_partner_id.name + ',' + '\n' + _("New lead assigned to you") + '\n*' + _("Lead name") + ':* ' + self.name + "" + message
        if signature:
